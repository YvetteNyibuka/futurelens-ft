...
export class HealthDataService {
  ...
  static async getHealthDataByType(
    ...
    try {
      ...
      // Directly use the simple format that the backend route expects
      // The backend route /:dataType/:year will handle mapping dataType to the correct dataset_type
      const response = await apiClient.get(
        `/health-data/${dataType}/${yearStr}`,
        {
          params: {
            limit,
            offset,
  ...
  static async getAvailableDataTypes(): Promise<{
    ...
    try {
      // Try the available endpoint first (simplified endpoint)
      const availableResponse = await apiClient.get("/health-data/available");
      if (availableResponse.data?.success) {
        const availableYears =
          availableResponse.data.data.available_years || [];
        const availableTypes = availableResponse.data.data.data_types || [];

        // Create combinations from the available types (extract base type from full type)
        const combinations = availableTypes.map((fullType: string) => {
        ...
        const uniqueTypes = [
          ...new Set(
            combinations.map(
              (c: { type: string; year: string; fullType: string }) => c.type
            )
          ),
        ];
      ...
      if (response.data?.success && response.data.data?.actual_combinations) {
        ...
        const types = [
          ...new Set(parsedCombinations.map((c: any) => c.type)),
        ] as string[];
        const years = [
          ...new Set(parsedCombinations.map((c: any) => c.year)),
        ] as string[];
  ...
  static async uploadDataFile(file: File): Promise<any> {
    try {
      ...
      if (fileWithPath.webkitRelativePath) {
        ...
        console.log("📁 Sending path info:", fileWithPath.webkitRelativePath);
      }
